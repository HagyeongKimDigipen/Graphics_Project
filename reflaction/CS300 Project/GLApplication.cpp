...
///////for shader
GLuint programID = 0;  //draw
GLuint programID2 = 0; //lamp
GLuint programID3 = 0; //skybox
GLuint programID4 = 0; //environment
...
//////////camera
glm::vec3 cameraPosition = glm::vec3(0.f, 0.f, 5.0f);
glm::vec3 cameraPosition2 = glm::vec3(0.f, 0.f, 5.0f);
...
glm::mat4 view = glm::lookAt(glm::vec3(0.f, 1.5f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f),glm::vec3(0.0f, 1.0f, 0.0f));
float radius = 10.0f;
...
{
    "../Common/skybox/right.jpg",
    "../Common/skybox/left.jpg",
    "../Common/skybox/top.jpg",
    "../Common/skybox/bottom.jpg",
    "../Common/skybox/front.jpg",
    "../Common/skybox/back.jpg",
};
...
GLfloat refractiveIndex = 0.f;
GLfloat chromaticAberration = 0.5f;
GLfloat fresnelPower = 0.5f;
GLfloat refractionMatirial = 1.333f;
...
int main()
{
    ...
    window = glfwCreateWindow(windowWidth, windowHeight, // window dimensions
        "CS300_Assignment3", // window title
    ...
    programID = mShader.LoadShaders("../Common/shaders/OldShader/CS300ShaderPhongShading.vert", "../Common/shaders/OldShader/CS300ShaderPhongShading.frag");
    programID2 = mShader2.LoadShaders("../Common/shaders/LampShader.vert", "../Common/shaders/LampShader.frag");
    programID3 = mShader3.LoadShaders("../Common/shaders/CS300SKYShader.vert", "../Common/shaders/CS300SKYShader.frag");
    programID4 = mShader4.LoadShaders("../Common/shaders/CS300EnvironmentShader.vert", "../Common/shaders/CS300EnvironmentShader.frag");
    
    mShader4.shaderUse(programID4);
    mShader3.shaderUse(programID3);
    ...
    std::array EnvironmentsMapping = {
    "Reflection",
    "Refraction",
    "Fresnel"
    };

    std::array RefractionMatrial = {
        "Air",
        "Hydrogen",
        "Water",
        "Olive Oil",
    ...
    do
    {
        ...
        setSkyBox();
        glUniformMatrix4fv(vertexTransform, 1, GL_FALSE, &skyBox->GetModelMatrix()[0][0]);
        skyBox->Display();
        ...
        glBindTexture(GL_TEXTURE_2D, rbo[0]);
        glUniform1i(glGetUniformLocation(programID4, "left"), 3);

        glActiveTexture(GL_TEXTURE4);
        glBindTexture(GL_TEXTURE_2D, rbo[1]);
        glUniform1i(glGetUniformLocation(programID4, "right"), 4);

        glActiveTexture(GL_TEXTURE5);
        glBindTexture(GL_TEXTURE_2D, rbo[2]);
        glUniform1i(glGetUniformLocation(programID4, "bottom"), 5);

        glActiveTexture(GL_TEXTURE6);
        glBindTexture(GL_TEXTURE_2D, rbo[3]);
        glUniform1i(glGetUniformLocation(programID4, "top"), 6);

        glActiveTexture(GL_TEXTURE7);
        glBindTexture(GL_TEXTURE_2D, rbo[4]);
        glUniform1i(glGetUniformLocation(programID4, "front"), 7);

        glActiveTexture(GL_TEXTURE8);
        glBindTexture(GL_TEXTURE_2D, rbo[5]);
        glUniform1i(glGetUniformLocation(programID4, "back"), 8);
        ///////////////////////////////////////////////////////////////////////
        ...
        
        {
            ...
            {
                if (ImGui::Combo("Environment Mapping", &checkMapping, EnvironmentsMapping.data(), static_cast<int>(EnvironmentsMapping.size())))
                {
                    if (checkMapping == 0)
                    {
                        mappingTypes = checkMapping;
                    }
                ...
                ImGui::Checkbox("using phong shading", &usePhong);
            ...
            if (checkMapping > 0)
            {
                ...
                {
                    ImGui::DragFloat("Refractive Index", &refractionMatirial, 0.1f, 0.1f, 20.f);

                    if (ImGui::Combo("Material", &matrialType, RefractionMatrial.data(), static_cast<int>(RefractionMatrial.size())))
                    {
                        if (matrialType == 0)
                        {
                            refractionMatirial = 1.000293f;
                        }
                        else if (matrialType == 1)
                        {
                            refractionMatirial = 1.000132f;
                        }
                        else if (matrialType == 2)
                        {
                            refractionMatirial = 1.333f;
                        }
                        else if (matrialType == 3)
                        {
                            refractionMatirial = 1.47f;
                        }
                        else if (matrialType == 4)
                        {
                            refractionMatirial = 1.31f;
                        }
                        else if (matrialType == 5)
                        {
                            refractionMatirial = 1.46f;
                        }
                        else if (matrialType == 6)
                        {
                            refractionMatirial = 2.42f;
                        }
                        else if (matrialType == 7)
                        {
                            refractionMatirial = 1.49f;
                        }
                    }

                    if (checkMapping == 2)
                    {
                        ImGui::DragFloat("Fresenel Power", &fresnelPower, 0.1f, 0.f, 5.f);
                    }

                    ImGui::Checkbox("using chromatic aberration", &usechromaticAberration);
                    {
                        if (usechromaticAberration == true)
                        {
                            ImGui::DragFloat("chromatic aberration", &chromaticAberration, 0.1f, 0.f, 5.f);
                        }
                    }
                    ImGui::TreePop();
                }
            ...
            {
                ImGui::DragFloat("Camera PositionX", &cameraPosition.x, 0.1f, -10.f, 10.f);
                ImGui::DragFloat("Camera PositionY", &cameraPosition.y, 0.1f, -10.f, 10.f);
                ImGui::DragFloat("Camera PositionZ", &cameraPosition.z, 0.1f, -10.f, 10.f);
...
void initMesh(int uvType, bool entity)
{
    ...
    skyBox = new DrawScene(windowWidth, windowHeight, DrawScene::mesh::skybox);
    ...
    centerObj->setscaleVector(glm::vec3(0.5f));
    centerObj->setCenter(glm::vec3(0.0f, 0.0f, 0.0f));
    centerObj->SetShader(programID4);
    textureShader = centerObj->myTexture.LoadTexture("../Common/textures/metal_roof_diff_512x512.png", programID4);
    textureShader2 = centerObj->myTexture.LoadTexture("../Common/textures/metal_roof_spec_512x512.png", programID4);
    ...
    skyBox->Init();
    skyBox->setscaleVector(glm::vec3(6.7f));
    skyBox->setCenter(glm::vec3(1.0f, 1.0f, 2.0f));
    textureShader3 = skyBox->myTexture.DrawSkyCub(faces);
}
...
void setShader()
{
    ...
    glm::mat4 camera = view;//glm::lookAt(glm::vec3(cameraPosition.x, cameraPosition.y, cameraPosition.z), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
    glm::mat4 projection = glm::perspective(glm::radians(60.f), 1024.f / 800.f, 0.1f, 100.f);
    ...
    //light
    lightPos = glGetUniformLocation(programID4, "lightPos");
    lgihtColor = glGetUniformLocation(programID2, "lightColor");
    ...
    curLightNum = glGetUniformLocation(programID4, "curLightNum");
    glUniform1i(curLightNum, lightNum);

    curLightType = glGetUniformLocation(programID4, "curLightType");
    glUniform1i(curLightType, lightTypes);

    mShininess = glGetUniformLocation(programID4, "material.shininess");
    glUniform1f(mShininess, 32.0f);

    mDiffuse = glGetUniformLocation(programID4, "material.diffuse");
    glUniform1i(mDiffuse, 0);

    mSpecular = glGetUniformLocation(programID4, "material.specular");
    glUniform1i(mSpecular, 1);

    ///////////
    checkUVs = glGetUniformLocation(programID4, "checkUV");
    glUniform1i(checkUVs, checkUV);

    GPUcalculations = glGetUniformLocation(programID4, "GPUcalculation");
    glUniform1i(GPUcalculations, GPUcalculation);

    checkEntitys = glGetUniformLocation(programID4, "checkEntity");
    glUniform1i(checkEntitys, checkEntity);

    emmisives = glGetUniformLocation(programID4, "emmisive");
    glUniform3f(emmisives, emmisive.x, emmisive.y, emmisive.z);

    coefficients = glGetUniformLocation(programID4, "coefficient");
    glUniform3f(coefficients, coefficient.x, coefficient.y, coefficient.z);

    gAmbients = glGetUniformLocation(programID4, "gAmbient");
    glUniform3f(gAmbients, gAmbient.x, gAmbient.y, gAmbient.z);

    uvMins = glGetUniformLocation(programID4, "uvMin");
    glUniform3f(uvMins, uvMin.x, uvMin.y, uvMin.z);

    uvMaxs = glGetUniformLocation(programID4, "uvMax");
    glUniform3f(uvMaxs, uvMax.x, uvMax.y, uvMax.z);

    fogs = glGetUniformLocation(programID4, "fog");
    glUniform3f(fogs, fog.x, fog.y, fog.z);

    fogMins = glGetUniformLocation(programID4, "fogMin");
    glUniform1f(fogMins, fogMin);

    fogMaxs = glGetUniformLocation(programID4, "fogMax");
    glUniform1f(fogMaxs, fogMax);

    isTextures  = glGetUniformLocation(programID4, "isTexture");
    glUniform1f(isTextures, isTexture);

    uvTypeChecks = glGetUniformLocation(programID4, "uvType");
    glUniform1i(uvTypeChecks, uvTypeCheck);

    glUniform1i(glGetUniformLocation(programID4, "usePhong"), usePhong);
    glUniform1i(glGetUniformLocation(programID4, "useChromaticAberration"), usechromaticAberration);
}
...
void setLightShader(int index)
{
    ...
    uniformLight = glGetUniformBlockIndex(programID4, "Lights");
    glUniformBlockBinding(programID4, uniformLight, 0);
...
void setSkyBox()
{
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP, textureShader3);

    skyboxss = glGetUniformLocation(programID3, "skyBoxs");
    glUniform1i(skyboxss, 2);

    vertexCamera = glGetUniformLocation(programID3, "camera");
    vertexProjection = glGetUniformLocation(programID3, "projection");
    vertexTransform = glGetUniformLocation(programID3, "transform");
    cameraPos = glGetUniformLocation(programID2, "cameraPos");

    glm::mat4 camera = glm::lookAt(glm::vec3(cameraPosition2.x, cameraPosition2.y, cameraPosition2.z), glm::vec3(0.f, 0.f, 0.f), glm::vec3(0, 1, 0));
    glm::mat4 projection = glm::perspective(glm::radians(90.f), 1024.f / 800.f, 0.1f, 10.f);
    glm::mat4 transform = glm::translate(glm::mat4(1.0f), lightPoss);
...
void setFBO()
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        glBindTexture(GL_TEXTURE_2D, rbo[i]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1024, 800, 0, GL_RGB, GL_UNSIGNED_BYTE, (void*)0);
...
void setCubmap()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, rbo[i], 0);
        ...
        vertexCamera = glGetUniformLocation(programID4, "camera");
        vertexProjection = glGetUniformLocation(programID4, "projection");
        vertexTransform = glGetUniformLocation(programID4, "transform");
        cameraPos = glGetUniformLocation(programID4, "cameraPos");

        glm::mat4 camera = view;// glm::lookAt(cameraPosition, cameraAim[i], cameraUp2[i]);
        ...
        refracIndex = glGetUniformLocation(programID4, "eta");
        chromatAbe = glGetUniformLocation(programID4, "chromaticAberration");
        fresnelP = glGetUniformLocation(programID4, "fresnelPower");

        glUniform1f(refracIndex, refractiveIndex);
        glUniform1f(chromatAbe, chromaticAberration);
        glUniform1f(fresnelP, fresnelPower);
        glUniform1f(glGetUniformLocation(programID4, "refractionM"), refractionMatirial);

        mappingType = glGetUniformLocation(programID4, "mappingType");
        glUniform1i(mappingType, mappingTypes);

        glm::mat4 camera2 = glm::lookAt(cameraPosition2, aim[i], upCamera[i]);

        //mShader3.shaderUse(programID3);
        glUseProgram(programID3);

        glDepthMask(GL_FALSE);
        glUniformMatrix4fv(glGetUniformLocation(programID3, "transform"), 1, GL_FALSE, &skyBox->GetModelMatrix()[0][0]);
        glUniformMatrix4fv(glGetUniformLocation(programID3, "camera"), 1, GL_FALSE, &camera2[0][0]);
        skyBox->Display();
        ...
        GLint vertexTransform2 = glGetUniformLocation(programID2, "transform");
        glUniformMatrix4fv(glGetUniformLocation(programID2, "camera"), 1, GL_FALSE, &camera2[0][0]);
        
        if (lightNum >= 1)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[0].x, diffuseUniform[0].y, diffuseUniform[0].z);
            myLamp->Display();
        }
        ...
        if (lightNum >= 2)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp2->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[1].x, diffuseUniform[1].y, diffuseUniform[1].z);
            myLamp2->Display();
        }
        ...
        if (lightNum >= 3)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp3->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[2].x, diffuseUniform[2].y, diffuseUniform[2].z);
        ...
        if (lightNum >= 4)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp4->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[3].x, diffuseUniform[3].y, diffuseUniform[3].z);
            myLamp4->Display();
        }
        ...
        if (lightNum >= 5)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp5->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[4].x, diffuseUniform[4].y, diffuseUniform[4].z);
            myLamp5->Display();
        }
        ...
        if (lightNum >= 6)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp6->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[5].x, diffuseUniform[5].y, diffuseUniform[5].z);
            myLamp6->Display();
        }
        ...
        if (lightNum >= 7)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp7->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[6].x, diffuseUniform[6].y, diffuseUniform[6].z);
            myLamp7->Display();
        }
        ...
        if (lightNum >= 8)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp8->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[7].x, diffuseUniform[7].y, diffuseUniform[7].z);
            myLamp8->Display();
        }
        ...
        if (lightNum >= 9)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp9->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[8].x, diffuseUniform[8].y, diffuseUniform[8].z);
            myLamp9->Display();
        }
        ...
        if (lightNum >= 10)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp10->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[9].x, diffuseUniform[9].y, diffuseUniform[9].z);
            myLamp10->Display();
        }
        ...
        if (lightNum >= 11)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp11->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[10].x, diffuseUniform[10].y, diffuseUniform[10].z);
            myLamp11->Display();
        }
        ...
        if (lightNum >= 12)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp12->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[11].x, diffuseUniform[11].y, diffuseUniform[11].z);
            myLamp12->Display();
        }
        ...
        if (lightNum >= 13)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp13->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[12].x, diffuseUniform[12].y, diffuseUniform[12].z);
            myLamp13->Display();
        }
        ...
        if (lightNum >= 14)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp14->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[13].x, diffuseUniform[13].y, diffuseUniform[13].z);
            myLamp14->Display();
        }
        ...
        if (lightNum >= 15)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp15->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[14].x, diffuseUniform[14].y, diffuseUniform[14].z);
            myLamp15->Display();
        }
        ...
        if (lightNum >= 16)
        {
            glUniformMatrix4fv(vertexTransform2, 1, GL_FALSE, &myLamp16->GetModelMatrix()[0][0]);
            glUniform3f(vertexColor, diffuseUniform[15].x, diffuseUniform[15].y, diffuseUniform[15].z);
            myLamp16->Display();
        }
...
void clear()
{
    ...
    glDeleteShader(programID2);
    glDeleteShader(programID3);
    glDeleteShader(programID4);
