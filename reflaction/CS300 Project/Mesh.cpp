...
int Mesh::calcVertexNormals(GLboolean bFlipNormals)
{
    ...
    for (; index < vertexIndices.size(); )
    {
        ...
        glm::vec3 middlePoint = glm::vec3((vertexBuffer[a].x + vB.x + vC.x) / 3.f, (vA.y + vB.y + vC.y) / 3.f, (vA.z + vB.z + vC.z) / 3.f);

        glm::vec3  E1face = vB - middlePoint;
        glm::vec3  E2face = vC - middlePoint;
        ...
        faceNormalsDisplay.at((static_cast<GLsizeiptr>(index / 3.0) - 1) * 2 + 1) = middlePoint + normalLength * Nface;
    }
