...
File Name: Mesh.cpp
Purpose: draw shape and calculate normal
...
struct compareVec
{
    ...
    {
        bool xCom = lhs.x == rhs.x;
        bool bCom = lhs.y == rhs.y;
        bool cCom = lhs.z == rhs.z;
...
int Mesh::calcVertexNormals(GLboolean bFlipNormals)
{
    ...
    vertexNormalDisplay.resize(static_cast<std::vector<glm::vec3, std::allocator<glm::vec3>>::size_type>(numVertices) * 2, glm::vec3(0.0f));
    ...
    for (; index < vertexIndices.size(); )
    {
        ...
        //new value for face normal
        float faceX = (vA.x + vB.x + vC.x) / 3.f;
        float faceY = (vA.y + vB.y + vC.y) / 3.f;
        float faceZ = (vA.z + vB.z + vC.z) / 3.f;

        glm::vec3 faceMiddle = glm::vec3(faceX, faceY, faceZ);

        glm::vec3  E1face = vB - faceMiddle;
        glm::vec3  E2face = vC - faceMiddle;

        glm::vec3  N = glm::normalize(glm::cross(E1, E2));
        glm::vec3  Nface = glm::normalize(glm::cross(E1face, E2face));
        ...
        faceNormalsDisplay.at((static_cast<GLsizeiptr>(index / 3.0f) - 1) * 2) = faceMiddle;
        faceNormalsDisplay.at((static_cast<GLsizeiptr>(index / 3.0f) - 1) * 2 + 1) = faceMiddle + normalLength * Nface;
    }

    // Now sum up the values per vertex
    for (int indexs = 0; indexs < vNormalSet.size(); ++indexs)
    {
        glm::vec3  vNormal(0.0f);
        setNormalLength(0.05f);

        auto nIt = vNormalSet[indexs].begin();
        while (nIt != vNormalSet[indexs].end())
        {
            vNormal += (*nIt);
            ++nIt;
        }
        ...
        // save vertex normal
        vertexNormals[indexs] = glm::normalize(vNormal);

        // save normal to display
        glm::vec3  vA = vertexBuffer[indexs];

        vertexNormalDisplay[2 * static_cast<std::vector<glm::vec3, std::allocator<glm::vec3>>::size_type>(indexs)] = vA;
        vertexNormalDisplay[(2 * static_cast<std::vector<glm::vec3, std::allocator<glm::vec3>>::size_type>(indexs)) + 1] = vA + (normalLength * vertexNormals[indexs]);
...
void Mesh::calcVertexNormalsForDisplay(GLboolean /*bFlipNormals*/)
{
    ...
    for (int iNormal = 0; iNormal < vertexNormals.size(); ++iNormal)
    {
        ...
        vertexNormalDisplay[2 * static_cast<std::vector<glm::vec3, std::allocator<glm::vec3>>::size_type>(iNormal)] = vertexBuffer[iNormal];
        vertexNormalDisplay[(2 * static_cast<std::vector<glm::vec3, std::allocator<glm::vec3>>::size_type>(iNormal)) + 1] = vertexBuffer[iNormal] + normal;
    }
}
...
int Mesh::calcUVs(Mesh::UVType uvType, bool entity)
{
    int rFlag = -1;
