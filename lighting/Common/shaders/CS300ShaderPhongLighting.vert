...
File Name: CS300ShaderPhongLighting.vert
Purpose: Phong lighting vertex shader
...
void main()
{
	vec3 FragPos = vec3(transform * vec4(vPosition.x, vPosition.y, vPosition.z, 1.0));
...
vec3 Direction(vec3 normal, vec3 viewDir, int i)
{
    ...
    vec3 ambients  = lights[i].ambient  * vec3(texture(material.diffuse, aTexCoord).rgb);

    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuses  = lights[i].diffuse  * diff * vec3(texture(material.diffuse, aTexCoord).rgb);

    float spec = pow(max(dot(viewDir, reflect(-lightDir, normal)), 0.0), material.shininess);
    vec3 speculars = lights[i].specular * spec * vec3(texture(material.specular, aTexCoord).rgb);
...
vec3 Point(vec3 normal, vec3 fragPos, vec3 viewDir, int i)
{
    ...
    vec3 ambient  = lights[i].ambient  * vec3(texture(material.diffuse, aTexCoord).rgb);

    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = lights[i].diffuse  * diff * vec3(texture(material.diffuse, aTexCoord).rgb);

    float spec = pow(max(dot(viewDir, reflect(-lightDir, normal)), 0.0), material.shininess);
    vec3 specular = lights[i].specular * spec * vec3(texture(material.specular, aTexCoord).rgb);
